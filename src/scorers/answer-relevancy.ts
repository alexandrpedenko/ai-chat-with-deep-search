...
export const checkAnswerRelevancy = async (opts: {
  ...
  if (statements.length === 0) {
    return {
      ...
      metadata: {
        statements: [],
        verdicts: [],
        reasoning: "No statements found in output",
      },
    };
  }
  ...
  const verdicts = verdictResult.object.verdicts;
  ...
  return {
    ...
    metadata: {
      statements,
      verdicts,
      reasoning: `Evaluated ${statements.length} statements with average relevancy score of ${averageScore}`,
    },
  };
};
